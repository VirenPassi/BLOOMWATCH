"""
Stage-1 MODIS Data Download for BloomWatch Pipeline

Downloads MODIS MOD13Q1 data for two AOIs covering different climates:
- AOI 1: California Central Valley (Mediterranean climate)
- AOI 2: Iowa Corn Belt (Continental climate)
...
# AOI Definitions (different climates)
AOIS = {
    "california_central_valley": {
        "name": "California Central Valley",
        "climate": "Mediterranean",
        "tiles": ["h08v05", "h09v05"],  # Central Valley tiles
        "description": "Agricultural region with Mediterranean climate"
    },
    "iowa_corn_belt": {
        "name": "Iowa Corn Belt", 
        "climate": "Continental",
        "tiles": ["h10v04", "h11v04"],  # Iowa tiles
...
def ensure_dirs():
    ...
    for aoi_name in AOIS.keys():
        (STAGE1_RAW_DIR / aoi_name).mkdir(exist_ok=True)
...
def download_modis_granule(aoi_name: str, tile: str, date: str, session: requests.Session) -> bool:
    ...
    try:
        ...
        save_path = STAGE1_RAW_DIR / aoi_name / filename
...
def main():
    ...
    print("Starting Stage-1 MODIS Data Download")
    print(f"Date range: {START_DATE.strftime('%Y-%m-%d')} to {END_DATE.strftime('%Y-%m-%d')}")
    print(f"Save directory: {STAGE1_RAW_DIR}")
    ...
    report = {
        ...
        "download_directory": str(STAGE1_RAW_DIR)
    }
    
    report_path = ROOT / "outputs" / "stage1_download_report.json"
    ...
    print(f"Data saved to: {STAGE1_RAW_DIR}")
