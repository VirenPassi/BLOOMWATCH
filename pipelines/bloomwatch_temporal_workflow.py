...
def load_nasa_modis_data(data_dir: Path) -> xr.Dataset:
    """Load real NASA MODIS data from downloaded files."""
    nasa_files = list(data_dir.glob("**/*.json"))
    if not nasa_files:
        raise FileNotFoundError("No NASA MODIS data files found")
    ...
    for file_path in nasa_files:
        ...
        # Create xarray dataset from NASA data
        ds = xr.Dataset({
            "ndvi": (["y", "x"], np.array(data["NDVI"])),
            "evi": (["y", "x"], np.array(data["EVI"])),
            "quality": (["y", "x"], np.array(data["Quality"]))
        })
        
        # Add time coordinate
        ds = ds.expand_dims("time")
        ds["time"] = [datetime.fromisoformat(data["timestamp"])]
